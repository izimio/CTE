...
function check_validity(elem) {
  if (elem == undefined || elem.length == 1) return 0;
  return 1;
}
...
exports.createUrl = (req, res, next) => {
  ...
  if (check_validity(url)) {
    db.execute(
      "INSERT INTO `content` (`url`, `password`) VALUES (?, ?)",
      [url, password],
...
exports.deleteUrl = (req, res, next) => {
  ...
  if (check_validity(url) && check_validity(password)) {
    db.execute(
      "DELETE FROM `content` WHERE `url` = ? AND `password` = ?",
      [url, password],
      function (err, results, fields) {
        if (!err) {
...
exports.modifyPassword = (req, res, next) => {
  ...
  if (
    (check_validity(url) && check_validity(password),
    check_validity(new_password))
  ) {
    db.execute(
      "UPDATE `content` SET `password` = ? WHERE `url` = ? AND password = ?",
      [new_password, url, password],
      function (err, results, fields) {
        if (!err) {
          res
            .status(401)
            .json({ statut: "Password succesfully modified for url : " + url });
          return;
        } else {
          res.status(500).json({ statut: "Not allowed" });
        }
...
exports.modifyContent = (req, res, next) => {
  ...
  if (
    (check_validity(url) && check_validity(password), check_validity(content))
  ) {
    db.execute(
      "UPDATE `content` SET `content` = ? WHERE `url` = ? AND password = ?",
...
exports.getUrl = (req, res) => {
  db.execute(
    "SELECT (content) FROM content WHERE url = ? AND password = ?",
    [req.query.url, req.query.password],
    function (err, results, fields) {
...
exports.checkPswd = (req, res, next) => {
  ...
  const password = req.body.password;
  db.execute(
    "SELECT (id) FROM content WHERE url = ? AND password = ?",
    [url, password],
    function (err, results, fields) {
      if (results && results[0]) {
